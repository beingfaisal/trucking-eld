...
export default function RouteInfo() {
  ...
  useEffect(() => {
    setFilters(types.reduce((o, t) => ({ ...o, [t]: true }), {}));
  }, [types]);

  const [activeEvent, setActiveEvent] = useState(null);
  useEffect(() => {
    if (activeEvent && !filters[activeEvent.type]) setActiveEvent(null);
  }, [filters, activeEvent]);

  useEffect(() => {
    if (!routeData) navigate('/', { replace: true });
  }, [routeData, navigate]);
      ...
      <main className="flex h-[calc(100vh-64px)] overflow-hidden">
        <aside className="w-64 shrink-0 overflow-auto bg-gray-50 p-4 space-y-6">
          <div className="bg-white p-4 rounded shadow">
            ...
            <p><strong>Distance:</strong> {totalMiles} miles</p>
            ...
            {events.map((evt, i) =>
              filters[evt.type] && (
                <Marker
                  ...
                  label={{ text: ICONS[evt.type], fontSize: '18px' }}
            ...
            {activeEvent && (
              ...
              >
                <div className="text-sm">
                  <h4 className="font-bold">{LABELS[activeEvent.type]}</h4>
                  {activeEvent.mile_marker   != null && <p>Mile: {activeEvent.mile_marker} mi</p>}
                  {activeEvent.arrival_time && (
                    <p>
                      Arrival:{' '}
                      {new Date(activeEvent.arrival_time).toLocaleString('en-GB', {
                        dateStyle: 'medium',
                        timeStyle: 'short'
                      })}
                    </p>
                  ...
                  {activeEvent.departure_time && (
                    <p>
                      Departure:{' '}
                      {new Date(activeEvent.departure_time).toLocaleString('en-GB', {
                        dateStyle: 'medium',
                        timeStyle: 'short'
                      })}
                    </p>
